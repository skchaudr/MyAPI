...
## 2026-04-12 - Prevent Information Disclosure through Unhandled Exceptions
**Vulnerability:** FastAPI endpoints returning `detail=str(e)` on unhandled `Exception`s leaked sensitive internal system states and tracebacks directly to the client, leading to Information Disclosure.
**Learning:** Default unhandled exception catch blocks that stringify the raw exception into HTTP responses violate the fail-secure principle.
**Prevention:** Instead of leaking the raw exception, always return a generic `500 Internal Server Error` message (e.g. `An internal server error occurred`) and properly log the exception details server-side using `logger.error("...", exc_info=True)`.

...
## 2026-04-14 - Fix DoS vulnerability in file uploads by chunking reads
**Vulnerability:** Fast API `UploadFile` was being read into memory all at once using `await file.read()`. This could lead to excessive memory consumption and Denial of Service (DoS) if an attacker uploaded an extremely large file.
**Learning:** When handling file uploads with FastAPI, never read the entire file into memory using `await file.read()`, as it's vulnerable to DoS attacks.
**Prevention:** Always process the data using chunked reads (e.g., `while chunk := await file.read(1024 * 1024):`) to keep memory usage bounded.

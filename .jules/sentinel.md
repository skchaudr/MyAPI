...
## 2026-08-06 - [Missing Authentication for Health Endpoint]
**Vulnerability:** The `/health` endpoint was exposed without any authentication, potentially leaking system metadata (like DB health, Gemini models, environment variables).
**Learning:** Even diagnostic endpoints should be secured, especially if they reveal internal topology, system availability, or backend metadata to untrusted actors.
**Prevention:** Implement `APIKeyHeader` and verify incoming keys securely using `secrets.compare_digest` to prevent timing attacks. Fail securely (e.g. 500 without details) if the server configuration itself is missing the key.

...
## 2024-07-29 - Generic Error Messages for Exceptions
**Vulnerability:** API endpoints were returning raw exception strings (e.g. `str(e)`) in 500 HTTP responses.
**Learning:** Returning raw exceptions can expose sensitive internal stack traces, configuration details, or other implementation specifics to the client.
**Prevention:** Always log exceptions securely on the backend using `logger.error(msg, exc_info=True)` and return a generic user-friendly string (e.g. "An internal server error occurred") in the HTTP response.

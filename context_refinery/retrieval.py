...
class ResultReranker:
    ...
    def rerank(self, results, intent="factual", query=None):
        ...
        # Build inverted index for reinforcement scoring
        all_tags = []
        tag_to_docs = {}
        ...
        for i, r in enumerate(results):
            ...
            reinf = self._reinforcement_score(i, all_tags[i], tag_to_docs, num_docs)
    ...
    def _reinforcement_score(self, index, my_tags, tag_to_docs, num_docs):
        """Boost if this doc's tags overlap with tags from other docs."""
        if not my_tags or num_docs <= 1:
            return 0.0
        ...
        return min(1.0, overlap_count / max(num_docs - 1, 1))

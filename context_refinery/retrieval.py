...
class ResultReranker:
    ...
    def rerank(self, results, intent="factual", query=None):
        ...
        # Collect all tags for reinforcement scoring, building an inverted index (tag_to_docs)
        # to maintain O(N) complexity and avoid severe CPU bottlenecks during large searches.
        all_tags = []
        tag_to_docs = {}
        ...
        for i, r in enumerate(results):
            ...
            reinf = self._reinforcement_score(i, all_tags[i], tag_to_docs, total_docs)
    ...
    def _reinforcement_score(self, index, my_tags, tag_to_docs, total_docs):
        ...
        if index in overlapping_docs:
            overlapping_docs.remove(index)

        # Normalize: more overlapping docs = higher score, capped at 1.0
        return min(1.0, len(overlapping_docs) / max(total_docs - 1, 1))

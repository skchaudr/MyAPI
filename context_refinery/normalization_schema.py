...
Defines the canonical metadata vocabulary for v1-stamped corpus items and
the deterministic path-based inference functions used by the
`scripts/normalize_corpus.py` CLI.

Design notes — see project-docs/corpus-v1-normalization-readiness.md and
project-docs/source-type-taxonomy-plan.md. The shape here is intentionally
narrow: stamping is path-driven where possible so 80% of items normalize
...
# Uses substring matches against the POSIX-style relative path.
...
# Conversation/CLI-session sources are keyed by adapter, not path.
_ADAPTER_TO_SOURCE_TYPE: dict[str, str] = {
...
def infer_source_type(path: str, adapter: Optional[str] = None) -> str:
    ...
    `path` is the canonical relative path the item lives under (POSIX style).
    `adapter` is one of {chatgpt, claude_web, claude_code, codex, obsidian};
    when set and not obsidian, it wins over path inference.
    """
    ...
    # Filename-based daily-note shortcut for Obsidian periodic notes
...
def infer_temporal_mode(source_type: str) -> str:
    ...
    Episodic items are events-as-they-happened (conversations, sessions,
    daily notes). Meta items are retrospective/curated. Hybrid is reserved
    for items that demonstrably blend both — defaulted at the call site,
    not here.
    """
...
def infer_primary_project(path: str, title: Optional[str] = None) -> str:
    ...
    Returns one of {myapi, devinfra, vault, gddp, openclaw, unknown}.
    Cheap heuristic — caller can override via review.
    """
...
# we only fill blanks. Keeps the operator's hand-curated frontmatter intact.
V1_FIELDS: tuple[str, ...] = (
    "source_type",
    "work_type",
    "temporal_mode",
...
class V1Stamp:
    ...
    def as_dict(self) -> dict:
        ...
        for k in ("thread_type", "outcome", "signal_strength",
                  "raw_thread_weight", "artifact_summary"):
            v = getattr(self, k)
            if v is not None:
                out[k] = v
        return out
...
def merge_v1_into_frontmatter(existing: dict, stamp: V1Stamp) -> dict:
    ...
    Existing values win over inferred ones — this preserves operator-curated
...
def stamp_from_path(
    ...
    *,
    ...
    """Build a V1Stamp for an item using path + adapter hints."""

...
async def import_obsidian(file: UploadFile = File(...)):
    ...
    try:
        ...
        logger.error("Error importing obsidian file: %s", e, exc_info=True)
...
async def import_chatgpt(file: UploadFile = File(...)):
    ...
    for item in data:
        try:
            ...
            logger.error("Skipping a conversation due to parsing error: %s", e, exc_info=True)
...
async def import_codex(request: CodexImportRequest = CodexImportRequest()):
    try:
        ...
        logger.error("Error scanning codex sessions: %s", e, exc_info=True)
        raise HTTPException(status_code=500, detail="An internal server error occurred")
...
async def import_claude_code(request: ClaudeCodeImportRequest = ClaudeCodeImportRequest()):
    try:
        ...
        logger.error("Error importing claude code sessions: %s", e, exc_info=True)
        raise HTTPException(status_code=500, detail="An internal server error occurred")

...
def validate_root_path(user_path: str, default_base: str) -> str:
    """Validates that the user_path does not traverse outside default_base."""
    base_dir = os.path.abspath(os.path.expanduser(default_base))
    # If user provided a path, resolve it relative to base_dir to support relative paths
    # like "subfolder", but typically they might provide an absolute path or relative path.
    if user_path:
        # Expand ~ if present
        expanded_user_path = os.path.expanduser(user_path)
        # If it's an absolute path, this works. If it's relative, it resolves against CWD.
        # But we really want to resolve relative paths against the base_dir.
        if not os.path.isabs(expanded_user_path):
             expanded_user_path = os.path.join(base_dir, expanded_user_path)
        target_path = os.path.abspath(expanded_user_path)
    else:
        target_path = base_dir

    if os.path.commonpath([base_dir, target_path]) != base_dir:
        raise ValueError("Invalid root path: path traversal detected.")
...
async def import_codex(request: CodexImportRequest = CodexImportRequest()):
    try:
        safe_root = validate_root_path(request.root, "~/.codex/command-logs")
...
async def import_claude_code(request: ClaudeCodeImportRequest = ClaudeCodeImportRequest()):
    try:
        safe_root = validate_root_path(request.root, "~/.claude/projects")
    except ValueError as e:
        logger.warning(f"Path traversal attempt blocked: {e}")
        raise HTTPException(status_code=400, detail="Invalid root path")

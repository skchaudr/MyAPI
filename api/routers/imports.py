...
async def import_codex(request: CodexImportRequest = CodexImportRequest()):
    # Prevent path traversal vulnerabilities by ensuring the resolved path is under the intended base directory.
    ...
    if os.path.commonpath([base_dir, target_path]) != base_dir:
        raise HTTPException(status_code=403, detail="Forbidden: Invalid root path.")

...
async def enrich_content(request: EnrichRequest):
    try:
        ...
        logger.error("Enrichment failed", exc_info=True)
        # Security: Prevent leaking internal error details to the client
...
async def enrich_batch(request: BatchEnrichRequest):
    ...
    for i, doc in enumerate(request.documents):
        try:
            ...
            if "GEMINI_API_KEY" in str(e):
                raise HTTPException(status_code=503, detail="GEMINI_API_KEY not configured on this server")
            logger.error(f"Batch enrichment failed for document index {i}", exc_info=True)
            # Security: Prevent leaking internal error details to the client
